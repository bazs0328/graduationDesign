...
### Base-P1.5（DeepTutor 借鉴：工程化增强）

> 重点补齐文档处理方式、效率与 LLM 交互精准度/流畅度，按投入产出比优先推进。  

- [ ] **文档处理阶段化进度（替代纯状态轮询）**
  - 范围：将文档处理拆为 `extract/chunk/index/lexical/done` 等 stage，返回 `progress_percent/current/total`；支持 SSE 或 WebSocket 推送
  - 验收标准：
    - `GET /api/docs/tasks` 可看到阶段、百分比、当前步骤信息
    - 前端可实时更新任务状态，不依赖高频轮询
    - 失败时可定位到具体阶段（而非仅 `error_message`）

- [ ] **KB 元数据并发安全（file_hash 写入加锁）**
  - 范围：`metadata.json` 读写加文件锁 + 原子写入，避免并发上传时 `file_hashes` 丢失或覆盖
  - 验收标准：
    - 并发上传场景下 `file_hashes` 无丢失
    - 异常中断后 metadata 文件保持可读
    - 关键路径具备并发 smoke 或单测覆盖

- [ ] **BM25 检索缓存与增量失效**
  - 范围：按 `user_id+kb_id` 缓存 tokenized corpus / BM25 对象；在 append/remove/move/rename 时触发增量失效
  - 验收标准：
    - QA 检索热路径延迟显著下降
    - 文档变更后检索结果可反映最新数据
    - 不出现跨用户/跨 KB 数据污染

- [ ] **统一 LLM 调用中间层（超时/重试/退避）**
  - 范围：封装 `llm_call` / `llm_stream`，统一处理 timeout、429/5xx backoff、错误映射与日志
  - 验收标准：
    - `summary/keypoints/quiz/qa` 不再散落直接调用 `invoke/ainvoke`
    - `max_retries/timeout` 可配置
    - 常见临时性错误可自动恢复

- [ ] **结构化输出鲁棒性升级（JSON 修复 + 能力探测）**
  - 范围：升级 `safe_json_loads`，支持 code block 提取、repair/fallback；对支持模型启用 `response_format`
  - 验收标准：
    - keypoints/quiz 的 JSON 解析失败率下降且可观测
    - 解析失败时保留原始响应片段用于排障
    - 对不支持 `response_format` 的模型自动降级

- [ ] **QA 流式回答（首字节反馈）**
  - 范围：后端新增 SSE/stream 接口，前端 QA 实时渲染增量 token，sources 在结束后回填
  - 验收标准：
    - 提问后可快速看到首段响应（网络正常条件下）
    - 回答过程中有增量更新的“生成中”反馈
    - 完成后 sources 与会话持久化逻辑保持一致
...
> **说明**：为避免重复，`UX-P0` 负责“基础设施”，`UX-P1` 负责“业务流程编排”，`UX-P2` 负责“性能与展示细节”。
...
- [ ] **流程上下文传递（跨页免重复选择）**
  - 范围：统一处理 `kb_id` / `doc_id` / `session_id` 路由上下文
  - 方案：路由 `query` + Pinia 已选状态兜底；目标页自动回填选择器
  - 验收标准：
    - Summary → Quiz 自动带 `doc_id`
    - Quiz → QA 自动带 `kb_id/doc_id`
    - Progress 点击 KB 统计后跳到对应 KB 视图

- [ ] **空状态引导优化**
  - 范围：统一空状态组件（图标 + 文案 + CTA）并按页面配置模板
  - 验收标准：
    - 列表/内容区空状态均包含“下一步动作”按钮
    - 新注册用户首次进入各页面均能明确知道下一步操作

- [ ] **操作安全与反馈一致性**
  - 范围：危险操作确认 + 成功后的可行动提示（在 Toast 基础上）
  - 方案：
    - 对清空/删除/重置统一 `ConfirmDialog`
    - 成功后统一 Toast 文案，并按场景提供下一步 CTA（如“去生成摘要”）
  - 验收标准：
    - 清空对话、删除文档、重置画像等操作均弹出确认
    - 确认框文案包含后果说明（如“不可撤销”）
    - 所有关键成功操作有统一提示，且至少覆盖 2 条跨页引导链路
...
- [ ] **响应式布局适配**
  - 范围：QA 侧栏（固定 `w-72`）在小屏下折叠；Quiz 网格在窄屏改单列；投影仪/低分辨率兼容
...
- [ ] **输入防抖与请求去重**
  - 范围：QA 连续提交防重入；搜索/筛选输入防抖（300ms）
  - 验收标准：快速连续触发仅发送一次有效请求；停止输入后再触发检索
